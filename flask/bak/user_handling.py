...
class User_handler:
    ...
    def find_user(self, user):  # -> tuple[bool, dict]: #FIXME: does not work on Pi
        ...
        """
        found_user = retrieve_data_from_db(
    ...
    def create_user(self, user) -> make_response:
        ...
        hashed_password = self.hash_password(
            user.password
        )  # Password obfuscation using hashing
    ...
    def change_password(self, user) -> make_response:
        ...
        Returns:
            response: flask response object
        """
        new_password = self.hash_password(
            user.password
        )  # Password obfuscation using hashing
    ...
    def retrieve_users(self, user) -> list:
        ...
        if user.role == "teacher":
            for id in self.user_db:
                user_list.append(
                    {
                        "id": self.user_db[id]["_id"],
                        "username": self.user_db[id]["username"],
                        "role": self.user_db[id]["role"],
                    }
                )
