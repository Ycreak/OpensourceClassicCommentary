...
class Bibliography_handler:
    ...
    def add_entry(self, bib_entry):
        ...
        if check_exist:
            ...
            for id in self.bib_db:
                ...
                if (
                    fuzz.ratio(bib_entry["author"], author) > EQUALITY_RATIO
                    and fuzz.ratio(bib_entry["title"], title) > EQUALITY_RATIO
                ):
                    return make_response(
                        f"Similar bibliography entry already exists: {author}, {title}",
                        403,
                    )

        # depending on the entry type, create a template.
        if bib_entry.bib_entry_type == "book":
            new_entry = copy.deepcopy(bib_entry.book_empty)
        elif bib_entry.bib_entry_type == "article":
            new_entry = copy.deepcopy(bib_entry.article_empty)
        elif bib_entry.bib_entry_type == "incollection":
            new_entry = copy.deepcopy(bib_entry.incollection_empty)
        else:
    ...
    def retrieve_bibliography_from_author(self, author):
        ...
        for bibliography in bibliographies:
            ...
            bib_entry = {
                "_id": bib_object.id,
                "bib_entry_type": bib_object.bib_entry_type,
            }

            if hasattr(bib_object, "author"):
                bib_entry["author"] = bib_object.author
            if hasattr(bib_object, "title"):
                bib_entry["title"] = bib_object.title
            if hasattr(bib_object, "year"):
                bib_entry["year"] = bib_object.year
            if hasattr(bib_object, "series"):
                bib_entry["series"] = bib_object.series
            if hasattr(bib_object, "number"):
                bib_entry["number"] = bib_object.number
            if hasattr(bib_object, "location"):
                bib_entry["location"] = bib_object.location
            if hasattr(bib_object, "edition"):
                bib_entry["edition"] = bib_object.edition
            if hasattr(bib_object, "journal"):
                bib_entry["journal"] = bib_object.journal
            if hasattr(bib_object, "volume"):
                bib_entry["volume"] = bib_object.volume
            if hasattr(bib_object, "pages"):
                bib_entry["pages"] = bib_object.pages
