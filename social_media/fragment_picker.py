...
class Fragment:
    ...
    def __init__(
        self, author: str, title: str, editor: str, name: str, latin: str, english: str
    ):
...
class FragmentPicker:
    ...
    def pick(self) -> Fragment:
        ...
        date_files.sort(
            key=lambda f: datetime.strptime(f[10:20], "%Y_%m_%d"), reverse=True
        )
        # Get the latest file
        ...
        for entry in data["rows"]:
            ...
            if (
                document["document_type"] == "fragment"
                and document["sandbox"] == "admin"
                and document["visible"] == 1
                and "translation" in document
        ...
        try:
            with open("assets/published_fragments.json", "r") as file:
        ...
        candidate_fragments = list(
            set(fragments_identifiers_list) - set(published_fragments)
        )
        ...
        fragment = next(
            (item for item in fragments_list if item["_id"] == fragment_id), None
        )
        ...
        if fragment:
            ...
            for line in fragment["lines"]:
                ...
                line_text = re.sub(
                    r"<(\d+)>", lambda match: " " * int(match.group(1)), line_text
                )
                latin += line_text + "\n"
