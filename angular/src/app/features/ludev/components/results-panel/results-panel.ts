...
export class ResultsPanelComponent {
  ...
  get activeFilterLabels(): string[] {
    ...
    if (f.maxDistance !== null && f.maxDistance !== this.FILTER_DEFAULTS.maxDistance) add(`Dist ≤ ${f.maxDistance}`);
    ...
    const confIsDefault =
      f.minConfidence === this.FILTER_DEFAULTS.minConfidence && f.maxConfidence === this.FILTER_DEFAULTS.maxConfidence;
  ...
  exportJson(): void {
    ...
    this.downloadFile(JSON.stringify(exportData, null, 2), this.getFilename('json'), 'application/json');
  }
  ...
  exportTxt(): void {
    ...
    this.downloadFile(`${header}\n${content}`, this.getFilename('txt'), 'text/plain');
  }
  ...
  private buildTextTable(headers: string[], rows: string[][]): string {
    ...
    const formatRow = (row: string[]): string => {
      return `| ${row.map((cell, columnIndex) => (cell ?? '').padEnd(columnWidths[columnIndex])).join(' | ')} |`;
    };
