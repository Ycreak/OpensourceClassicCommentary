...
export class FabricService {
  ...
  private get_viewport_bounds(): { min_x: number; min_y: number; width: number; height: number; cx: number; cy: number } {
    ...
    const width = this.canvas.width! / this.canvas.getZoom();
    const height = this.canvas.height! / this.canvas.getZoom();
  ...
  public clear_zones(): void {
    const zone_objects = this.canvas.getObjects().filter((obj) => (obj as any).is_zone === true);
    zone_objects.forEach((obj) => this.canvas.remove(obj));
  ...
  public evaluate_step(zones: DropZone[]): EvaluationResult[] {
    const documents = this.canvas.getObjects().filter((obj) => this.is_document(obj));
  ...
  private clear_feedback_labels(): void {
    const labels = this.canvas.getObjects().filter((obj) => (obj as any).is_feedback_label === true);
  ...
  public reset_fragments_to_initial(initial_state: any): void {
    this.canvas.loadFromJSON(initial_state, () => this.canvas.renderAll());
  }
