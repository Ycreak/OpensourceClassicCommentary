...
import { DropZone } from './lesson.model';

describe('TeachingFabricService', () => {
  let service: TeachingFabricService;
  let fabric_service: FabricService;

  beforeEach(() => {
    TestBed.configureTestingModule({});
    fabric_service = TestBed.inject(FabricService);
    service = TestBed.inject(TeachingFabricService);
  });
  ...
  describe('evaluate_step', () => {
    ...
    /**
     * Stub the underlying FabricService's canvas with the bare minimum API surface
     * used by `evaluate_step`: a `getObjects()` returning the supplied fragments.
     */
    const stub_canvas = (objects: any[]): void => {
      (fabric_service as any).canvas = {
        getObjects: () => objects,
      };
    };
