...
export class FragmentsComponent implements OnInit {
  ...
  private request_titles(column: Fragment_column): void {
    this.api.get_titles(column.author).subscribe((data) => {
  ...
  private request_editors(column: Fragment_column): void {
    this.api.get_editors(column.author, column.title).subscribe((data) => {
  ...
  private request_fragments(column: Fragment_column): void {
    ...
    this.api.get_fragments(api_data).subscribe((fragment_list) => {
      // Format the data just how we want it
      fragment_list = this.add_HTML_to_lines(fragment_list);
      fragment_list = fragment_list.sort(
        this.utility.sort_fragment_array_numerically
      );
      ...
      if (column.name != 'playground') {
        ...
        if (this.columns.length > 0) {
          this.columns[
            this.columns.findIndex((i) => i.column_id === column.column_id)
          ] = column;
        }
      } else {
   ...
   * @author Ycreak
   */
  private handle_author_selection(
   ...
   * @author Ycreak
   */
  private handle_editor_selection(
    column: Fragment_column,
    editor: string
  ): void {
  ...
  public add_column(): void {
    ...
    let new_fragment_column = new Fragment_column(
  ...
  public move_column(column_id, direction): void {
    ...
    if (to_index >= 0 && to_index < this.columns.length) {
      this.columns = this.move_element_in_array(
        this.columns,
        from_index,
        to_index
      );
   ...
   */
  public delete_clicked_item_from_playground(
    column: Fragment_column,
    item: string
  ): void {
    if (item == 'fragment') {
  ...
  private colour_linked_fragments(fragment: Fragment): void {
    ...
    for (let i in fragment.linked_fragments) {
      ...
      for (let j in this.columns) {
        ...
        let corresponding_fragment = this.columns[j].fragments.find(
          (i) => i.fragment_id === linked_fragment_id
        );
        // colour it if found
  ...
  private sort_fragments_on_status(fragments: Fragment[]): Fragment[] {
    let normal = this.utility.filter_object_on_key(
      fragments,
      'status',
      'Certum'
    );
    let incerta = this.utility.filter_object_on_key(
      fragments,
      'status',
      'Incertum'
    );
    let adesp = this.utility.filter_object_on_key(
      fragments,
      'status',
      'Adesp.'
    );
   ...
   *
   */
  private add_single_fragment_to_playground(
    column: Fragment_column,
    fragment_name
  ): void {
  ...
  private add_content_to_current_fragment(fragment): void {
    if (fragment['translation'] != '') {
      this.current_fragment.translation = fragment['translation'];
    }
    if (fragment['differences'] != '') {
      this.current_fragment.differences = fragment['differences'];
    }
    if (fragment['apparatus'] != '') {
      this.current_fragment.apparatus = fragment['apparatus'];
    }
    if (fragment['commentary'] != '') {
      this.current_fragment.commentary = fragment['commentary'];
    }
    if (fragment['reconstruction'] != '') {
      this.current_fragment.reconstruction = fragment['reconstruction'];
    }
    if (fragment['context'] != '') {
      this.current_fragment.context = fragment['context'];
    }
    if (fragment['bibliography'] != '') {
      this.current_fragment.bibliography = fragment['bibliography'];
    }
  }
}
