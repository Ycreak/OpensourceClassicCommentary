...
export class Multiplayer {
  constructor(private firestore: AngularFirestore, private utility: UtilityService) {
  ...
  ngOnInit(): void {
    //FIXME: why is this not called upon class creation?
  ...
  public InitiateFirestore(session, table) {
    // Create watcher
    this.firestore_session = this.firestore
      .collection(table)
      .valueChanges()
      .subscribe((data) => {
        ...
        this.firestore
          ...
          .subscribe((ss) => {
            ...
            ss.docs.forEach((doc) => {
              if (doc.id == session) {
                // Only select our session document
  ...
  public DeleteFirebaseSession(session, table) {
    ...
    this.firestore.collection(table).doc(session).delete();
