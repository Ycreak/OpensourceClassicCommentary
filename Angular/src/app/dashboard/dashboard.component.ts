...
export class DashboardComponent implements OnInit {
  ...
  user_table_columns_to_displayWithExpand = [
    ...this.user_table_columns_to_display,
    'expand',
  ];
  ...
  public request_wysiwyg_dialog(field: string, index: number = 0): void {
    if (field == 'context') {
      ...
      this.dialog
        .open_wysiwyg_dialog(form_array_field, this.WYSIWYG_editor_config)
        .subscribe((result) => {
          ...
          let context_array = this.fragment_form.controls[
            'context'
          ] as FormArray;
      ...
      this.dialog
        .open_wysiwyg_dialog(
          this.fragment_form.value[field],
          this.WYSIWYG_editor_config
        )
        .subscribe((result) => {
   ...
   * @author Ycreak
   */
  public push_fragment_line_to_fragment_form(
    line_number: string,
    text: string
  ): void {
   ...
   * @author Ycreak
   */
  public push_fragment_context_to_fragment_form(
    _author: string,
    _location: string,
    _text: string
  ): void {
   ...
   * @author Ycreak
   */
  public remove_form_item_from_form_array(
    form_name: string,
    target: string,
    index: number
  ): void {
  ...
  public request_titles(author: string): void {
    ...
    this.api.get_titles(author).subscribe((data) => {
  ...
  public request_editors(author: string, title: string): void {
    ...
    this.api.get_editors(author, title).subscribe((data) => {
   ...
   */
  public retrieve_requested_fragment(
    author: string,
    title: string,
    editor: string,
    ...
    api_data.author = author;
    api_data.title = title;
    api_data.editor = editor;
    api_data.fragment_name = fragment_name;

    this.api.get_specific_fragment(api_data).subscribe((fragment) => {
  ...
  public request_fragment_names(
    ...
    this.api.get_fragment_names(api_data).subscribe((data) => {
      this.retrieved_fragment_names = data.sort(
  ...
  public request_revise_fragment(fragment_form: FormGroup): void {
    ...
    if (fragment_form.value.lock) {
      //&& !this.auth_service.current_user.role == 'teacher') {
      ...
      let item_string =
        ...
        ': ' +
        fragment_form.value.fragment_name;

      this.dialog
        .open_confirmation_dialog(
          'Are you sure you want to REVISE this fragment?',
          item_string
        )
        .subscribe((result) => {
          if (result) {
            this.api
              .revise_fragment(fragment_form.value)
              .subscribe(this.fragment_edit_observer);
          }
        });
    }
  }
  ...
  public request_create_fragment(fragment_form: FormGroup): void {
    ...
    this.dialog
      .open_confirmation_dialog(
        'Are you sure you want to CREATE this fragment?',
        item_string
      )
      .subscribe((result) => {
        if (result) {
          this.api
            .create_fragment(fragment_form.value)
            .subscribe(this.fragment_edit_observer);
        }
      });
  }
  ...
  public request_delete_fragment(fragment_form: FormGroup): void {
    ...
    this.dialog
      ...
      .subscribe((result) => {
        if (result) {
          this.api
            .delete_fragment(fragment_form.value)
            .subscribe(this.fragment_edit_observer);
        }
      });
  }
   ...
   */
  public request_automatic_fragment_linker(
    author: string,
    title: string
  ): void {
    ...
    this.dialog
      .open_confirmation_dialog(
        'Are you sure you want to LINK fragments from this text?',
        item_string
      )
      .subscribe((result) => {
        if (result) {
  ...
  public request_users() {
    ...
    api_data.role = this.auth_service.current_user_role;
    api_data.username = this.auth_service.current_user_name;
    ...
    this.dialog
      ...
      .subscribe((result) => {
        if (result) {
          ...
          this.api
            .user_change_role(api_data)
            .subscribe(this.user_edit_observer);
        }
      });
  }
      ...
      this.dialog
        ...
        .subscribe((result) => {
          if (result) {
            ...
            api_data.password = form.value.password1;
  ...
  public push_bibliography_reference(bib_entry) {
    let bibliography_references = this.fragment_form.get(
      'linked_bib_entries'
    ) as UntypedFormArray;
  ...
  public on_bibliography_tab_change(tab_change_event: MatTabChangeEvent): void {
    this.update_form_field(
      'bibliography_form',
      'bib_entry_type',
      tab_change_event.tab.textLabel.toLowerCase()
    );
  }
  ...
  public handle_bib_entry_selection(bib_entry) {
    ...
    if (bib_entry.bib_entry_type == 'book')
      this.bibliography_form_selected_type.setValue(0);
    if (bib_entry.bib_entry_type == 'article')
      this.bibliography_form_selected_type.setValue(1);
  ...
  public request_bibliography_authors() {
    this.api.get_bibliography_authors().subscribe({
      next: (data) => {
        this.bibliography_author_selection_form_options =
          this.push_bibliography_authors_in_list(data); //TODO: this need to be handled with a model
      },
      error: (err) => this.utility.handle_error_message(err),
    });
  }
  ...
  public request_bibliography_from_id(id) {
    this.api.get_bibliography_from_id(id).subscribe((data) => {
      ...
      temp = data;
      let linked_bib_entries = this.fragment_form.get(
        'linked_bib_entries'
      ) as UntypedFormArray;
  ...
  public request_revise_bibliography_entry(bibliography) {
    ...
    this.dialog
      .open_confirmation_dialog(
        'Are you sure you want to REVISE this bibliography entry?',
        item_string
      )
      .subscribe((result) => {
        if (result) {
          this.api
            .revise_bibliography_entry(bibliography)
            .subscribe(this.bibliography_edit_observer);
        }
      });
  ...
  public request_create_bibliography_entry(bibliography) {
    ...
    this.dialog
      .open_confirmation_dialog(
        'Are you sure you want to CREATE this bibliography entry?',
        item_string
      )
      .subscribe((result) => {
        if (result) {
          this.api
            .create_bibliography_entry(bibliography)
            .subscribe(this.bibliography_edit_observer);
        }
      });
    // this.reset_form('bib_form');
  ...
  public request_delete_bibliography_entry(bibliography) {
    ...
    this.dialog
      .open_confirmation_dialog(
        'Are you sure you want to DELETE this bibliography entry?',
        item_string
      )
      .subscribe((result) => {
        if (result) {
          this.api
            .delete_bibliography_entry({ _id: bibliography.id })
            .subscribe(this.bibliography_edit_observer);
        }
      });
    // this.reset_form('bib_form');
